...
declare variable $tower := 
fn:tokenize("gbyvdfh (155) -> xqmnq, iyoqt, dimle
oweiea (97)
szhxrs (14)
pjvwsiw (23)
...
declare variable $bottoms := ();
declare variable $leaves := ();
declare variable $base-pro := "";

declare variable $tower-map := map:new();

declare function local:get-program($pk as xs:string)
{
  $tower[fn:starts-with(fn:normalize-space(.), $pk)]
};

declare function local:get-weight($programs as xs:string*)
{
  let $all-weight := 
    for $item in $programs
    return 
    (
      let $program := local:get-program($item)
      return
      (
        let $weight := fn:normalize-space(fn:substring-after(fn:substring-before($program, ")"), "("))
        return 
        if (functx:is-a-number($weight)) then ( fn:number($weight) ) else ( 0 )
      )
    )
  return fn:sum($all-weight)
};

declare function local:parse-program($pk as xs:string)
{
  let $program := local:get-program($pk)
  let $base := fn:normalize-space(fn:substring-before($program, "("))
  let $weight := fn:number(fn:normalize-space(fn:substring-after(fn:substring-before($program, ")"), "(")))
  let $base-cont := fn:tokenize(fn:substring-after($program, "->"), ",") ! fn:normalize-space(.)
  let $base-cont-weight := local:get-weight($base-cont)
  let $l := if ($pk eq $base-pro)
            then ( "tbp" )
            else if (fn:exists($base-cont)) 
            then ( "bp" )
            else ( "lp" )
  let $sub-programs := 
            for $b in $base-cont
            return local:parse-program($b)
  let $bcw := fn:sum($sub-programs/@tw ! xs:integer(.) )
  let $tw := fn:sum(($weight, $sub-programs/@tw ! xs:integer(.)))
  return 
    <p n="{$base}" w="{$weight}" l="{$l}" bcw="{$bcw}" tw="{$tw}">
    {
       $sub-programs
    }
    </p>
...
declare function local:find-base-program()
{
  let $tower-base :=
    $tower ! 
    (
    if (fn:matches(., "([a-z]*)\s+\([0-9]+\)\s+->\s+[(a-z,\s+)]*")) then (

      functx:get-matches(., "([a-z]*)\s+\([0-9]+\)\s+->\s+[(a-z,\s+)]+")
    ) else ()
    )
  let $_ := 
    for $tower in $tower-base
    let $base := fn:normalize-space(fn:substring-before($tower, "("))
    let $base-cont := fn:tokenize(fn:substring-after($tower, "->"), ",") ! fn:normalize-space(.)
    return 
    (
      xdmp:set($bottoms, ($bottoms, $base))
      ,
      xdmp:set($leaves, ($leaves, $base-cont))
    )
  return xdmp:set($base-pro, functx:value-except($bottoms, $leaves))
};

local:find-base-program()
,
let $data :=
<t>
{
  local:parse-program($base-pro)
}
</t>
return
let $unbalanced-programs-base := (($data//p[fn:not(@tw = (preceding-sibling::p/@tw, following-sibling::p/@tw))])[fn:last() - 1])
let $weights := fn:distinct-values($unbalanced-programs-base/p/@tw ! xs:integer(.))
let $max := fn:max($weights)
let $min := fn:min($weights)  
let $diff-weight := $max - $min
let $unbalanced-program := 
  if (fn:count($weights[. eq $max]) eq 1)
  then ( $unbalanced-programs-base/p[@tw eq $max])
  else ( $unbalanced-programs-base/p[@tw eq $min])
return 
( 
"1. Base program of tower: " || $base-pro
,
"2. Unbalanced Program "
,
"   Current weight of program '" || $unbalanced-program/@n || "': " || $unbalanced-program/@w || ", over-weight by " || $diff-weight || " discs"
,
"   Weight required to balance : " || ($unbalanced-program/@w - $diff-weight)
)
