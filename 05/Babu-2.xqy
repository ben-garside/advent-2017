...
{
  let $keys := functx:sort(map:keys($maze-map))
  return 
  $keys ! map:get($maze-map, .)
};

declare function local:seq-to-map($maze)
{
  let $map := map:map()
  let $_ := (
    for $item at $pos in $maze 
    return map:put($map, xs:string($pos), $item)
    )
  return (
    xdmp:set($maze-map, $map)
    ,
    xdmp:set($maze-map-size, fn:count(map:keys($maze-map)))
  )
};

(
local:seq-to-map($maze)
,
local:move-map(1)
,
"Total Steps " || $STEPS
