...
declare function local:char-count-sorted($input as xs:string*)
as xs:string*
{
  for $str in $input
  let $pp-trans := 
    for $p in fn:tokenize($str, " ")
    return
    fn:string-join(
        for $str-char in fn:distinct-values(functx:sort(functx:chars($p)))
        return fn:concat($str-char, functx:number-of-matches($p, $str-char))
    , "")
  return fn:string-join($pp-trans, " ")
...
let $pp := fn:tokenize($input, "&#xa;")
let $trans-input := local:char-count-sorted($pp)
let $run := 
  for $trans-line at $pos in $trans-input
  let $tokens := fn:tokenize(fn:normalize-space($trans-line), " ")
  return 
  (
  "Input: {" || $pp[$pos] || "}, >> Trans. input {" || $trans-line || "}, Status: " || (if (fn:count($tokens) eq fn:count(fn:distinct-values($tokens))) 
  then (
      xdmp:set($valid-count, ($valid-count + 1)) 
      ,
      ": Valid"
  ) 
  else ": Invalid")
...
$run
,
"Total valid Passphrase: " || $valid-count
)
