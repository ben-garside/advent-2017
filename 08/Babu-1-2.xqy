...
declare variable $ins := 
fn:tokenize("kd dec -37 if gm <= 9
x dec -715 if kjn == 0
ey inc 249 if x < 722
n dec 970 if t > 3
f dec -385 if msg > -3
...
declare variable $regs := map:new();
declare variable $highest := 0;
declare function local:parse-ins($ins as xs:string)
{
  let $reg-opr := fn:substring-before($ins, " if")
  let $reg-con := fn:substring-after($ins, "if ")
  return (
    fn:normalize-space($reg-opr)
    ,
    fn:normalize-space($reg-con)
  )

};

declare function local:get-registers($ins as xs:string*)
{
  fn:distinct-values($ins ! (fn:substring-before(., " "))) ! map:put($regs, ., 0)
};

declare function local:inc($reg as xs:string, $value as xs:integer)
{
  map:put($regs, $reg, (map:get($regs, $reg) + $value))
};

declare function local:dec($reg as xs:string, $value as xs:integer)
{
  map:put($regs, $reg, (map:get($regs, $reg) - $value))
};

declare function local:fix-opr($opr)
{
  switch ($opr)
  case "==" return "="
  default return $opr
...
declare function local:manage-highest()
{
  let $curr-higest := fn:max(<m>{$regs}</m>//*:value/data())
  return
    if ($highest lt $curr-higest)
    then (
      xdmp:set($highest, $curr-higest)
    )
    else ()
};
...
let $_ := local:get-registers($ins)
return
(
for $entry in $ins
  let $reg-entry := local:parse-ins($entry)
  let $reg-opr := $reg-entry[1]
  let $reg-con := $reg-entry[2]
  let $tokens := fn:tokenize($reg-opr, " ") ! fn:normalize-space(.)
  let $reg := $tokens[1]
  let $fun := $tokens[2]
  let $val := xs:integer($tokens[3])
  let $reg-val := map:get($regs, $reg)

  let $tokens2 := fn:tokenize($reg-con, " ") ! fn:normalize-space(.)
  let $c-reg := $tokens2[1]
  let $c-opr := $tokens2[2]
  let $c-val := $tokens2[3]
  let $c-reg-val := map:get($regs, $c-reg)
  
  let $eval := xs:string($c-reg-val||local:fix-opr($c-opr)||$c-val)
  return 
  (
    if(xdmp:eval($eval)) then (
      let $function := xdmp:function(xs:QName("local:" || $fun))
      return(
        xdmp:apply($function, $reg, $val)
      )  
    ) else ()
    ,
    local:manage-highest()
  )
  ,
  "Max value in a register: " || fn:max(<m>{$regs}</m>//*:value/data())
  ,
  "Highest value in a register: " || $highest
)
